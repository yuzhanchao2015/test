...
说明： common.js文件加载完成会自动创建F对象，我们可以用这样的方式使用F的方法
		F.toast('我是F', 1500, '48vw')
		F.setCookie('F',666,'13d');
		方法的参数介绍看下面
		注意commonJs文件要在使用F对象的js文件之前加载，就像你的jQ要在其他js之前加载一样
...
tirmSpace: 去掉字符串两端的空格，第一个参数为要处理的字符串，第二个参数有三个选项'left' 'right 'all' 分别代表删除左侧空格、删除右侧空格和删除字符串中的全部空格  当第二个参数为空或者传递的为其他字符串时，将删除要处理的字符串的两端的空格
